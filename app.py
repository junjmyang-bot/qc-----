...
# --- 3. 데이터 로직 및 세션 상태 (에러 방지를 위한 개별 초기화) ---
...
if 'history' not in st.session_state: st.session_state.history = {k: [] for k in ["a4","a5","b3","b4","b5","b9","a8","b2","b6","b7","b8","b10","a1","a2","a3","a6","a7","a9","b1"]}
if 'a4_ts' not in st.session_state: st.session_state.a4_ts = []
if 'a8_logs' not in st.session_state: st.session_state.a8_logs = [] # [해결] AttributeError 방지 로직
...
with st.sidebar:
    ...
    with st.expander("📅 시프트 루틴 설정 (최상단)", expanded=True):
        ...
        st.divider(); st.info("📦 Bahan Baku (Shift 1 Focus)")
...
with st.container(border=True):
    ...
    with cb:
        ...
        if sw_b1: st.markdown(f"**B1. {QC_CONTENT['B']['b1']['title']}**"); st.pills("b1", ["Awal", "Istirahat"], selection_mode="multi", key="u_b1")
    ...
    with col_a:
        ...
        if sw_a8: # A-8 인지 강화 타이핑 로직
            ...
            if curr_a8 < 8:
                ...
                f_inf = {}
                if has_f == "Yes":
                    f_inf['p'] = st.text_input("Nama produk", key=f"a8_p_{curr_a8}")
                    f_inf['k'] = st.text_input("Berapa kg / pcs", key=f"a8_k_{curr_a8}")
                    f_inf['r'] = st.text_area("Alasan jatuh", key=f"a8_re_{curr_a8}")
                
                # 'YES'를 정확히 쳐야만 확인 버튼 활성화
...
if st.button("💾 저장 및 텔레그램 전송", type="primary", use_container_width=True):
    try:
        ...
        # (기존 상세 리포트 투사 로직 실행)
        send_telegram(tg_msg); st.success("✅ 리포트 전송 완료!")
