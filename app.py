...
# [수정] 기본 타겟 설정을 0으로 하여 준모님이 직접 입력하게 유도
...
with st.sidebar:
    ...
    # [혁신] 준모님이 직접 숫자를 써서 타겟을 정하는 섹션
    with st.expander("🎯 오늘의 리포트 목표 설정", expanded=False):
        st.info("오늘 수행할 리포트 횟수를 직접 입력하세요.")
        for k in GRID_KEYS:
            name = QC_CONTENT['A' if 'a' in k else 'B'][k]['title']
            # number_input을 사용하여 준모님이 직접 입력
            st.session_state.targets[k] = st.number_input(f"{k.upper()} 목표 횟수", min_value=0, max_value=48, value=st.session_state.targets[k], key=f"inp_{k}")
            st.caption(f"현재 설정: {st.session_state.targets[k]}회")
