...
# --- 2. [데이터 보존] 19개 전 항목 가이드 및 상세 질문 데이터 ---
QC_CONTENT = {
    "A": {
        ...
        "a9": {"title": "SISA BARANG", "qs": ["Check MAX 1 PACK (Sudah check/Belum)", "Sisa shift prev?", "Sudah dibereskan?", "Simpan sisa?", "Handover sisa?"]},
        "a4": {"title": "Laporan QC di tablet", "check_items": ["daily kebersihan", "kontaminan kupas", "kontaminan packing"]},
        "a5": {"title": "Status tes steam", "desc": ["maksimal jam 13.00", "update laporan 30분 마다", "cek sampel", "cek laporan"]},
    ...
    "B": {
        "b1": {"title": "Cek Laporan Absensi", "desc": ["Durasi 2 kali awal masuk과 후식 후", "Perubahan jumlah orang 확인"], "areas": ["Steam", "Dry", "Kupas", "Packing"]},
...
with st.sidebar:
    ...
    with st.expander("📅 시프트 루틴 설정", expanded=True):
        ...
        sw_a3=st.toggle(f"A-3", True); sw_a7=st.toggle(f"A-7", True); sw_a9=st.toggle(f"A-9", True)
...
with st.container(border=True):
    ...
    with cola:
        ...
        if sw_a3:
            st.markdown(f"**A3. Handover shift 전**")
        ...
        if sw_a7:
            st.markdown(f"**A7. Handover & rencana**")
        ...
        if sw_a6:
            st.markdown(f"**A6. List BB butuh kirim**")
    ...
    with colb:
        ...
        # [수정] B-1 Absensi 라벨링: Setelah Istirahat 반영
        if sw_b1:
...
if st.button("💾 저장 및 텔레그램 전송", type="primary", use_container_width=True):
    try:
        ...
        if sw_b1:
            ...
            for tl in ["Awal Masuk", "Setelah Istirahat"]:
                ...
                for ar in QC_CONTENT['B']['b1']['areas']:
                    ...
                    tg_msg += f"  - {ar}: {d['jam'] if d['jam'] else '00.00'} / {d['pax'] if d['pax'] else '0 pax'} / ({d['st']})\n"
        ...
        send_telegram(tg_msg); st.success("✅ 상세 리포트 전송 완료!")
