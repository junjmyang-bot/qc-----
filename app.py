...
with st.sidebar:
    ...
    with st.expander("📅 Rutinitas Shift", expanded=True):
        sw_a1=st.toggle(f"A-1 Stok BB Steam", True); sw_a2=st.toggle(f"A-2 Stok BS Defros", True)
        sw_a3=st.toggle("A-3 Handover Masuk", True); sw_a7=st.toggle("A-7 Rencana Prod", True); sw_a9=st.toggle("A-9 Sisa Barang", True)
        st.divider(); sw_a5=st.toggle(f"A-5 Tes Steam", True); sw_a6=st.toggle("A-6 List BB", True)
        st.divider(); sw_b1=st.toggle("B-1 Absensi", True)
    with st.expander("⚡ 30 Menit (Target)", expanded=False):
        for k in ["a4", "b3", "b4", "b5", "b9"]:
            t_sw = st.toggle(f"Aktifkan {k.upper()}", True, key=f"sw_{k}")
            if t_sw: st.session_state.targets[k] = st.number_input(f"Target {k.upper()}", 0, 48, st.session_state.targets[k], key=f"inp_{k}")
    with st.expander("⏰ 1 Jam (Target)", expanded=False):
        for k in ["a8", "b2", "b6", "b7", "b8", "b10"]:
            t_sw = st.toggle(f"Aktifkan {k.upper()}", True, key=f"sw_{k}")
            if t_sw: st.session_state.targets[k] = st.number_input(f"Target {k.upper()}", 0, 24, st.session_state.targets[k], key=f"inp_{k}")
...
with st.container(border=True):
    ...
    with ca:
        ...
        if sw_a1:
            ...
            p_a1 = st.pills("Waktu A1", ["Awal Masuk", "Setelah Istirahat"], selection_mode="multi", key="u_a1")
...
for tit, keys, a_ks, b_ks in [("⚡ Interval 30 Menit", ["a4","b3","b4","b5","b9"], ["a4"], ["b3","b4","b5","b9"]), ("⏰ Interval 1 Jam", ["a8","b2","b6","b7","b8","b10"], ["a8"], ["b2","b6","b7","b8","b10"])]:
    st.subheader(tit); with st.container(border=True):
        ca, cb = st.columns(2)
        with ca:
            st.info("🅰️ QC Direct Check")
            for k in a_ks:
                if st.session_state.get(f"sw_{k}", True) and st.session_state.targets[k] > 0:
                    ...
                    if k == "a8" and st.session_state.get("active_a8"):
                        with st.expander("Verifikasi A-8", expanded=True):
                            if st.text_input("Barang dibereskan? (Type 'YES')", key="a8_v").strip().upper() == "YES" and st.button("Confirm"):
        ...
        with cb:
            ...
            for k in b_ks:
                if st.session_state.get(f"sw_{k}", True) and st.session_state.targets[k] > 0:
                    ...
                    for q in QC_CONTENT['B'][k]['qs']: st.markdown(f"<span style='color:black; font-size:0.85rem;'>✓ {q}</span>", unsafe_allow_html=True)
...
main_memo = st.text_area("Input Catatan Tambahan", key="main_memo_v")

# --- 6. [RESTORASI] Kirim ke Telegram (A + B Full Laporan) ---
if st.button("💾 SIMPAN & KIRIM LAPORAN KE TELEGRAM", type="primary", use_container_width=True):
    try:
        ...
        # [A: Rutinitas QC] - 복구 완료
        tg_msg += "📅 *Rutinitas QC (Direct Check)*\n"
        if sw_a1: tg_msg += f"• A-1. Stok BB: {ans_a1_1 if ans_a1_1 else '-'}\n"
        if sw_a7: tg_msg += f"• A-7. Handover: {ans_a7_1}\n  └ Stok: {ans_a7_3 if ans_a7_3 else '-'}\n"
        if sw_a5: tg_msg += f"• A-5. Tes Steam: {ans_a5}\n"
        
        # [A-4, A-8 Interval Check] - 복구 완료
